	...
	<div class="workflow-canvas">
		...
		>
			<template #edge-custom="props">
				<CustomEdge v-bind="props" @add-node="handleEdgeAddNode" />
			</template>
			<template #edge-loop="props">
				<LoopEdge v-bind="props" @add-node="handleEdgeAddNode" />
			</template>
...
const placeholderOutputIndex = ref<number>(0); // Track which output index for multi-output nodes
...
const flowNodes = computed<Node[]>({
	get: () => {
		...
		workflowStore.nodes.forEach((node) => {
			...
			// Get node metadata
			const metadata = nodeTypeRegistry.get(node.type);
			if (!metadata) return;

			// Handle placeholder creation based on strategy
			const connections = workflowStore.connections[node.name]?.main || [];

			if (metadata.placeholderStrategy === 'multi-output') {
				// For multi-output nodes (e.g., loop), create placeholder for first output only
				const hasDoneConnection = connections[0] && connections[0].length > 0;
				if (!hasDoneConnection && metadata.outputs[0]) {
					nodes.push({
						id: `placeholder-done-${node.id}`,
						type: 'placeholder',
						position: {
							x: node.position[0] + 280,
							y: node.position[1] + metadata.outputs[0].handleOffset * 1.2, // Scale offset for positioning
						},
						data: {
							sourceNodeId: node.id,
							sourceOutputIndex: 0,
							outputName: metadata.outputs[0].displayName,
							isPlaceholder: true,
						},
						sourcePosition: 'right' as const,
						targetPosition: 'left' as const,
				...
				// Standard nodes: Add placeholder if no outgoing connection
				const hasOutgoingConnection = connections.some((conns) => conns && conns.length > 0);
			...
			// 'none' strategy: no placeholder created
		});
...
const flowEdges = computed<Edge[]>({
	get: () => {
		...
		const findNodeByName = (name: string) => workflowStore.nodes.find((n) => n.name === name);
...
function handleNodeClick(event: any) {
	...
	if (nodeData.isPlaceholder) {
		...
		// Store the output index if it's a multi-output placeholder
		if (nodeData.sourceOutputIndex !== undefined) {
			placeholderOutputIndex.value = nodeData.sourceOutputIndex;
		}
...
function handleNodeSave(nodeData: any) {
	if (insertEdgeData.value) {
		...
		if (sourceNode) {
			// Position based on output index for multi-output nodes
			const metadata = nodeTypeRegistry.get(sourceNode.type);
			const outputConfig = metadata?.outputs[placeholderOutputIndex.value];
			const yOffset = outputConfig ? outputConfig.handleOffset * 1.2 : 0;

			nodeData.position = [sourceNode.position[0] + 280, sourceNode.position[1] + yOffset];

			// Add new node
			workflowStore.addNode(nodeData);

			// Connect source to new node using the correct output index
			workflowStore.addConnection(sourceNode.name, placeholderOutputIndex.value, nodeData.name, 0);
		}

		placeholderSourceId.value = null;
		placeholderOutputIndex.value = 0;
	} else {
