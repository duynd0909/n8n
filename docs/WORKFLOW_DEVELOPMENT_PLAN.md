...
### 9.3 Node Configuration Panel Example

```vue
<!-- components/nodes/PostgresNodeConfig.vue -->
<template>
  <a-drawer
    :open="visible"
  ...
  >
    <a-form :model="formState" layout="vertical">
      <a-form-item label="Operation" name="operation" required>
        <a-select v-model:value="formState.operation">
          ...
          <a-select-option value="createTable">Create Table</a-select-option>
        </a-select>
      </a-form-item>

      <a-form-item label="Table" name="table" required>
        <a-input v-model:value="formState.table" placeholder="table_name" />
      </a-form-item>

      <a-form-item v-if="formState.operation === 'select'" label="Columns" name="columns">
        <a-input v-model:value="formState.columns" placeholder="*" />
      </a-form-item>

      <a-form-item label="Where Conditions">
        <a-space direction="vertical" style="width: 100%">
          <a-card
            v-for="(condition, index) in formState.whereConditions"
            :key="index"
            size="small"
          >
            <a-space>
              <a-input v-model:value="condition.field" placeholder="field" style="width: 100px" />
              <a-select v-model:value="condition.operator" style="width: 80px">
                <a-select-option value="=">=</a-select-option>
                <a-select-option value="!=">!=</a-select-option>
                <a-select-option value=">">></a-select-option>
                <a-select-option value="<"><</a-select-option>
              </a-select>
              <a-input v-model:value="condition.value" placeholder="value" style="width: 100px" />
      ...
      <a-form-item label="Raw Query" name="query">
        <a-textarea
...
const formState = reactive({
  ...
  columns: '*',
  query: '',
  whereConditions: [] as Array<{ field: string; operator: string; value: string }>
});

watch(() => props.node, (node) => {
  if (node) {
    Object.assign(formState, node.parameters);
  }
}, { immediate: true });

function addCondition() {
  formState.whereConditions.push({ field: '', operator: '=', value: '' });
}

function removeCondition(index: number) {
  formState.whereConditions.splice(index, 1);
}
...
### 9.4 Workflow List Example

```vue
<!-- views/WorkflowList.vue -->
<template>
  <a-card title="Workflows">
    <template #extra>
    ...
    <a-table :columns="columns" :data-source="workflows" :loading="loading" row-key="id">
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'active'">
          <a-switch
            :checked="record.active"
            @change="toggleActive(record)"
            :loading="record.activating"
          />
        </template>
        <template v-if="column.key === 'status'">
          <a-tag :color="getStatusColor(record.lastExecutionStatus)">
            {{ record.lastExecutionStatus || 'Never run' }}
          </a-tag>
        </template>
        <template v-if="column.key === 'actions'">
          <a-space>
            <a-tooltip title="Edit">
              <a-button type="text" @click="editWorkflow(record)">
                <EditOutlined />
              </a-button>
            </a-tooltip>
            <a-tooltip title="Execute">
              <a-button type="text" @click="executeWorkflow(record)">
                <PlayCircleOutlined />
              </a-button>
            </a-tooltip>
            <a-popconfirm
              title="Delete this workflow?"
              @confirm="deleteWorkflow(record)"
            >
...
const columns = [
  ...
  { title: 'Last Run', key: 'status', width: 120 },
...
function editWorkflow(record: any) {
  router.push(`/workflows/${record.id}`);
}

async function executeWorkflow(record: any) {
  try {
    await workflowApi.executeWorkflow(record.id);
...
async function toggleActive(record: any) {
  ...
  try {
    if (record.active) {
      await workflowApi.deactivateWorkflow(record.id);
    } else {
      await workflowApi.activateWorkflow(record.id);
    }
...
async function deleteWorkflow(record: any) {
