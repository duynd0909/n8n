...
export const useExecutionStore = defineStore('execution', () => {
	...
	function updateNodeStatus(nodeId: string, status: 'pending' | 'running' | 'success' | 'error') {
		nodeStatuses.value.set(nodeId, status);
	}

	function addNodeData(executionId: string, nodeName: string, taskData: ITaskData) {
	...
	function finishExecution(executionId: string, status: 'success' | 'error' | 'canceled') {
		for (const execs of executions.value.values()) {
			const execution = execs.find((e) => e.id === executionId);
			if (execution) {
