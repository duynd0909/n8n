					...
					<div class="summary-content">
						<div v-if="currentExecution" class="summary-grid">
							...
							<div v-if="currentExecution.finishedAt" class="summary-item">
								...
								<span>{{
									calculateDuration(currentExecution.startedAt, currentExecution.finishedAt)
								}}</span>
							</div>

							<div class="summary-item">
								<label>Nodes Executed</label>
								<span>{{ Object.keys(currentExecution.runData).length }} / {{ totalNodes }}</span>
							</div>
						</div>
					...
					<div class="node-logs-content">
						<div class="node-logs-layout">
							...
							<div class="node-list">
								...
								>
									<div class="node-status-indicator">
										<CheckCircleOutlined v-if="getNodeStatus(node.id) === 'success'" />
										<CloseCircleOutlined v-else-if="getNodeStatus(node.id) === 'error'" />
										<LoadingOutlined v-else-if="getNodeStatus(node.id) === 'running'" />
							...
							<div class="node-log-details">
								...
								<div v-else class="no-node-selected">Select a node to view logs</div>
							</div>
						</div>
					</div>
				</a-tab-pane>
				...
				<a-tab-pane key="raw" tab="Raw">
					<div class="raw-logs-content">
						...
						>
							...
							<span :class="['raw-level', log.level]">{{ log.level.toUpperCase() }}</span>
							<span class="raw-message">{{ log.message }}</span>
						</div>

						<div v-if="executionStore.executionLogs.length === 0" class="no-logs">
							No logs available
						</div>
					</div>
				</a-tab-pane>
...
const executionSummary = computed(() => {
	...
	const statusText = status === 'success' ? 'Success' : status === 'error' ? 'Error' : 'Running';
	return duration ? `Last run: ${statusText} · ${duration}` : `Status: ${statusText}`;
});
