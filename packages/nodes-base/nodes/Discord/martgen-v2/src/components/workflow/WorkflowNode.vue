		...
		>
			...
			<template v-if="hasMultipleOutputs">
				...
				>
					...
					/>
					<div class="output-label" :style="getOutputLabelStyle(outputIndex - 1)">
						{{ getOutputName(outputIndex - 1) }}
					</div>
				</div>
			</template>
...
const isTriggerNode = computed(() => {
	return props.data.type === 'schedule' || props.data.type === 'manual';
});
...
const nodeDisplayNameMap: Record<string, string> = {
	postgres: 'PostgreSQL',
...
// Output names for nodes with multiple outputs
const nodeOutputNames: Record<string, string[]> = {
	loop: ['done', 'loop'],
	if: ['true', 'false'],
	switch: [], // Dynamic based on cases
};
...
function getOutputName(index: number): string {
	const names = nodeOutputNames[props.data.type];
	return names?.[index] || `output ${index}`;
}

function getOutputHandleStyle(index: number) {
	const baseTop = 50; // Center position
	const offset = 25; // Offset from center

	let top;
	if (outputCount.value === 2) {
		top = index === 0 ? baseTop - offset : baseTop + offset;
	} else {
		top = baseTop;
	}

	// Color coding for loop node
	let backgroundColor = '#8c8c8c';
	if (props.data.type === 'loop') {
		backgroundColor = index === 0 ? '#52c41a' : '#fa8c16'; // done=green, loop=orange
	} else if (props.data.type === 'if') {
		backgroundColor = index === 0 ? '#52c41a' : '#ff4d4f'; // true=green, false=red
	}
...
const nodeIcon = computed(() => nodeIconMap[props.data.type] || DatabaseOutlined);
const nodeColor = computed(() => nodeColorMap[props.data.type] || '#666');
...
const nodeTemplateForModal = computed(() => ({
	...
	displayName: nodeDisplayNameMap[props.data.type] || props.data.type,
	description: '',
	color: nodeColorMap[props.data.type] || '#666',
}));
